...
object HttpConst {
    ...
    const val HTTP_FEATURE_IMAGE_CACHE = true
    const val HTTP_FEATURE_HTTPS_CERT = false
    const val HTTP_FEATURE_HTTPS_ALL = false
    const val HTTP_FEATURE_TOKEN_VALIDATE = true

    const val HTTP_GET = 1
    const val HTTP_POST = 2
    const val HTTP_PUT = 3
    const val HTTP_DELETE = 4
    const val HTTP_FILE_UPLOAD = 5
    const val HTTP_MULTIPART = 6

    const val HTTP_CONNECT_TIMEOUT = 10
    const val HTTP_READ_TIMEOUT = 30
    const val HTTP_RETRY_COUNT_MAX = 3
    ...
    const val HTTP_MIME_TYPE_IMG = "image/*"
    ...
    const val HTTP_MIME_TYPE_FORM = "application/x-www-form-urlencoded"
    const val HTTP_MIME_TYPE_MULTIPART = "multipart/form-data"
    const val HTTP_MIME_TYPE_M3U8 = "application/x-mpegURL"

    const val HTTP_MULTIPART_TEXT = "text-part"
    ...
    enum class Error(private val nCode: Int, private val strMsg: String) {
        ...
        HTTP_UNKNOWN_HOST(-9, "HTTP Unknown Host"),
        HTTP_IO_EXCEPTION(-10, "HTTP IO Exception"),
        HTTP_FILE_NOT_FOUND(-11, "HTTP File Not Found");
